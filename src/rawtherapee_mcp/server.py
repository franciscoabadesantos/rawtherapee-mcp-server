...
_DEBUG_MANUAL_TOOLS_ENV = "RT_ENABLE_DEBUG_MANUAL_TOOLS"
_NORMAL_MODE_HIDDEN_TOOLS = frozenset(
    {
...
def _debug_manual_tools_enabled() -> bool:
    return os.environ.get(_DEBUG_MANUAL_TOOLS_ENV, "").strip().lower() in {"1", "true", "yes", "on"}
...
def _profile_provenance_path(profile_path: Path) -> Path:
...
def _write_profile_provenance(
    *,
    ...
    payload = {
        "profile_provenance": {
            "generated_by": "auto_edit_manifest_select_prepare",
            "workflow": "manifest_select",
            "profile_id": uuid4().hex,
            "raw_path": str(raw_path.resolve()),
            "profile_path": str(profile_path.resolve()),
            "created_at": datetime.now(UTC).isoformat(),
            "manifest_validation": "passed" if validation.get("allowed") else "failed",
        }
    }
...
def _load_profile_provenance(profile_path: Path) -> dict[str, Any] | None:
    provenance_path = _profile_provenance_path(profile_path)
...
def _validate_profile_provenance(*, raw_path: Path, profile_path: Path) -> tuple[bool, dict[str, Any] | None, str | None]:
    ...
    if provenance.get("generated_by") != "auto_edit_manifest_select_prepare":
        return False, payload, "Profile was not generated by auto_edit_manifest_select_prepare."
    if provenance.get("workflow") != "manifest_select":
        return False, payload, "Profile provenance workflow is not manifest_select."
    if provenance.get("manifest_validation") != "passed":
        return False, payload, "Profile provenance does not record passed manifest validation."
    if str(provenance.get("raw_path")) != str(raw_path.resolve()):
        return False, payload, "Profile provenance raw_path does not match the requested RAW file."
    if str(provenance.get("profile_path")) != str(profile_path.resolve()):
        return False, payload, "Profile provenance profile_path does not match the requested profile."
...
def _verify_required_scores(verification_observations: dict[str, Any]) -> list[str]:
    scores = verification_observations.get("scores", {})
    if not isinstance(scores, dict):
        return list(_VERIFY_SCORE_FIELDS)
    missing: list[str] = []
    for field in _VERIFY_SCORE_FIELDS:
...
async def _prepare_manifest_select_packet(
    ...
    *,
    ...
    provenance_payload = _write_profile_provenance(
        raw_path=source_raw,
    ...
    return {
        ...
        "parameters": parameters,
        "validation": translated.get("validation", {}),
        "profile_provenance_path": str(_profile_provenance_path(Path(profile_path))),
        "profile_provenance": provenance_payload.get("profile_provenance", {}),
...
def _artifact_term_is_negated(text: str, term: str) -> bool:
    text_lower = text.lower()
    for match in re.finditer(re.escape(term), text_lower):
        prefix = text_lower[max(0, match.start() - 64) : match.start()]
        tokens = re.findall(r"[a-z']+", prefix)
        normalized_tokens = [token.replace("'", "") for token in tokens]
        if any(token in _NEGATION_TOKENS for token in normalized_tokens[-3:]):
            return True
        if any(
            phrase in prefix
            for phrase in (
...
def _find_asserted_artifact_terms(text_parts: list[str]) -> list[str]:
    asserted: list[str] = []
    for term in _NEGATIVE_ARTIFACT_TERMS:
        for text in text_parts:
            lowered = text.lower()
            if term not in lowered:
                continue
            if _artifact_term_is_negated(lowered, term):
                continue
...
def _consistency_checks(
    ...
    asserted_terms = _find_asserted_artifact_terms(text_parts)
    if asserted_terms:
        joined_terms = ", ".join(asserted_terms)
        flagged_values = (
                ...
                warnings.append(
                    {
                        "field": field,
                        "reason": (
                            f"Descriptions mention artifact-like terms ({joined_terms}) "
            ...
            warnings.append(
                {
                    ...
                    "reason": f"Descriptions mention artifact-like terms ({joined_terms}) but artifact_check is pass",
                }
            )
...
async def verify_predictive_edit(
    ...
    provenance_ok, provenance_payload, provenance_error = _validate_profile_provenance(
        raw_path=source_raw,
        profile_path=profile.resolve(),
    )
    if not provenance_ok:
        return {
            "error": "invalid_profile_provenance",
            "reason": "verify_predictive_edit only accepts profiles generated by auto_edit_manifest_select_prepare.",
            "details": provenance_error,
            "profile_path": str(profile),
            "profile_provenance_path": str(_profile_provenance_path(profile)),
            "profile_provenance": provenance_payload,
            "required_workflow": _verification_required_workflow(),
        }
    ...
    missing_score_fields = _verify_required_scores(verification_observations)
    if missing_score_fields:
        return {
            "error": "verification_scores_required",
            "missing_score_fields": missing_score_fields,
            "reason": "Verification descriptions are not enough. Numeric score fields are required before final decision.",
        }
    ...
    if decision in {"proof_only", "failed_edit_quality", "crop_only_improvement"}:
        ...
        output["required_workflow"] = _verification_required_workflow()
    ...
    output["profile_provenance_path"] = str(_profile_provenance_path(profile))
    output["profile_provenance"] = provenance_payload.get("profile_provenance", {})
    return await _tool_result_with_preview_images(
        output,
        max_width=preview_width,
...
async def _list_tools_with_normal_mode_filter(*args: Any, **kwargs: Any) -> Any:
    tools = await _original_mcp_list_tools(*args, **kwargs)
    if _debug_manual_tools_enabled():
        return tools
    return [tool for tool in tools if getattr(tool, "name", None) not in _NORMAL_MODE_HIDDEN_TOOLS]


async def _call_tool_with_normal_mode_filter(
    name: str,
    ...
    *,
    ...
    if not _debug_manual_tools_enabled() and name in _NORMAL_MODE_HIDDEN_TOOLS:
        msg = f"Tool not available in normal autonomous mode: {name}"
...
mcp.list_tools = _list_tools_with_normal_mode_filter
mcp.call_tool = _call_tool_with_normal_mode_filter
